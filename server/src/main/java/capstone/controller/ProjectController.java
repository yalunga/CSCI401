...
public class ProjectController {
  ...
  @CrossOrigin
  public @ResponseBody List<Project> getProjectsFromSemester(@PathVariable("semester") int semester,
  ...
  public @ResponseBody Project getUserProject(@PathVariable("email") String email) {
    try {
      ...
      return projectService.findByProjectId(user.getProjectId());
  ...
  public List<Project> projectAssignment() {
    ...
    /*
     * // WIP: Return an existing matching if students have already been assigned to
     * projects List<Project> existing = projectService.getExistingAssignments(); if
     * (existing != null && existing.size() > 0) { return existing; }
     */
    return projectService.runAlgorithm();
  }

  @GetMapping("/assignment-retrieve/{semester}/{fallspring}")
  @CrossOrigin
  public List<Project> retrieveProjectAssignment(@PathVariable("semester") int semester,
      @PathVariable("fallspring") int fallSpring) {
    return projectService.getExistingAssignments(semester, fallSpring);
  }

  @GetMapping("/assignment/exists/{semester}/{fallspring}")
  @CrossOrigin
  public String assignmentExists(@PathVariable("semester") int semester, @PathVariable("fallspring") int fallSpring) {
    List<Project> existing = projectService.getExistingAssignments(semester, fallSpring);
  ...
  public @ResponseBody String projectRankingsSubmission(@PathVariable("email") String email,
      @RequestBody List<Ranking> projects) {
  ...
  @CrossOrigin
  public @ResponseBody String saveData(@PathVariable("email") String email, @RequestBody Project project) {
  ...
  public @ResponseBody String rejectProjects(@PathVariable("projectId") int projectId) {
    ...
    String email = stakeholder.getEmail();
    emailService.sendEmail("Project Proposal Rejected",
        "Your project proposal has been rejected. Please feel free to reach out regarding any questions.", email);
    return Constants.SUCCESS;
  }

  // @PostMapping("/update-info")
  // @CrossOrigin
  // public @ResponseBody String updateProjectInfo(@RequestBody Map<String,
  // String> info) {
  // System.out.println("in the update project info");

  // String projectId = info.get("projectId");
  // Project project =
  // projectService.findByProjectId(Integer.parseInt(projectId));
  //
  //
  ...
  public @ResponseBody String requestChangeAdminCommentProjects(@PathVariable("projectId") int projectId,
    ...
    String email = stakeholder.getEmail();
    emailService.sendEmail("Requested Changes On Project Proposal",
        "There has been changes requested on your project proposal: <strong>" + adminComments + "</strong>", email);
    return Constants.SUCCESS;
  }
