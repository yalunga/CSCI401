...
export default class Stakeholder extends React.Component<StakeholderRegistrationProps, StakeholderRegistrationState> {
  ...
  async submitClicked() {
    ...
    try {
      await fetch(`${process.env.REACT_APP_API_URL}/users/stakeholder-registration`, {
        method: 'POST',
        body: data,
        headers: {
  ...
  render() {
    const { firstName, lastName, email, phone, company, password, confirm, loading, accountCreated } = this.state;
    return (
      <Box gap='small' pad='small'>
        <Text weight='bold' size='large' margin={{ top: 'small' }}>Stakeholder Registration</Text>
          ...
          <Box elevation='xsmall' round='xxsmall' width='medium' pad='small' gap='small'>
            {!loading ? (
              <Box gap='small'>
                ...
                <Box gap='xxxsmall'>
                  <Text size='small' color='dark-4'>Company/Organization</Text>
                  <Box border='bottom'>
                    <TextInput
                      plain
                      style={{ padding: 1 }}
